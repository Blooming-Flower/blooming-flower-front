...
const TuiEditor = ({ content = "", editorRef, height, placeholder }: Props) => {
  const createCustomFunc = (type: string) => {
    const cont = editorRef.current
      .getInstance()
      .getSelectedText(
        editorRef.current.getInstance().getSelection()[0],
        editorRef.current.getInstance().getSelection()[1]
      );
    if (type == "U") {
      editorRef.current
        .getInstance()
  ...
  const createCustomButton = (param: string) => {
    ...
    button.innerHTML =
      param == "U"
        ...
        : param == "B"
        ? "<div style='border: 1px solid black; height: 80%; width: 100%; border-radius: 5px'></div>"
