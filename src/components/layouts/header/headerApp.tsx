...
import {CssBaseline, Drawer, Hidden, useScrollTrigger} from '@mui/material'
import logo from '@images/common/urflowerMini.png'
...
const HeaderApp = () => {
	...
	const triggerStyle = {
		backgroundColor: useScrollTrigger() ? 'rgba(255,255,255,0.13)' : 'rgba(255,247,226,0.9)',
				...
				<AppBar elevation={useScrollTrigger() ? 24 : 0} style={triggerStyle}>
					<Toolbar>
						<IconButton size="large" edge="start" color="inherit" aria-label="menu" sx={{ mr: 2 }} onClick={toggleDrawer(true)}>
							<MenuIcon style={{ fontSize: '40px', color:'rgb(255,194,144)' }} />
						</IconButton>
