...
const HeaderDrawer = ({ toggleDrawer }: any) => {
	const QUESTION = [PATH.QUESTION0, PATH.QUESTION1, PATH.QUESTION2, PATH.QUESTION3, PATH.QUESTION4, PATH.QUESTION5]
	return (
		<Box sx={{ width: 200 }} role="presentation" onKeyDown={toggleDrawer(false)}>
			<List component="div" disablePadding
				...
				{['지문저장', '지문관리', '문제출제', '시험지제작', '시험지보기', '시험지관리'].map((text, index) => (
					<ListItem key={text} disablePadding>
						<ListItemButton sx={{ pl: 4 }} component={Link} to={QUESTION[index]}>
							<ListItemIcon>
								{index === 0 ? (
									<RecordVoiceOverIcon />
								) : index === 1 ? (
